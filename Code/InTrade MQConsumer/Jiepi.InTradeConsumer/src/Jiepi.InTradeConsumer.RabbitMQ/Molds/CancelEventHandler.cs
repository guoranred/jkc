...
{
    public class CancelEventHandler : IDistributedEventHandler<CancelMoldEto>, ITransientDependency
    {
        ...
        public async Task HandleEventAsync(CancelMoldEto eventData)
        {
        ...
        [UnitOfWork]
        protected virtual async Task<bool> ValidAsync(CancelMoldEto eventData)
        {         
            if (eventData == null)
            {
            ...
            if (order.Status >= (int)EnumMoldOrderStatus.SureOrder)
            {
        ...
        [UnitOfWork]
        protected virtual async Task CancelOrderAsync(CancelMoldEto eventData)
        {
            var entity = await _orderRepository.FindAsync(t => t.OrderNo == eventData.OrderNo);
            entity.SetStaus((int)EnumMoldOrderStatus.Cancel);
            await _orderRepository.UpdateAsync(entity);
        }

        [UnitOfWork]
        protected virtual async Task<int> CanceleOrderDetailAsync(CancelMoldEto eventData)
        {
