...
namespace Jiepi.InTradeConsumer.Service.Cncs
{
    ...
    /// 订单报价
    /// </summary>
    public class OfferEventHandler : IDistributedEventHandler<OfferCncEto>, ITransientDependency
    {
        ...
        public async Task HandleEventAsync(OfferCncEto eventData)
        {
        ...
        [UnitOfWork]
        protected virtual async Task<bool> ValidAsync(OfferCncEto eventData)
        {
            if (eventData == null)
            {
            ...
            if (order.Status > (int)EnumCncOrderStatus.OfferComplete)
            {
        ...
        [UnitOfWork]
        protected virtual async Task ChangeOrderStatusAsync(OfferCncEto eventData)
        {
            var entity = await _orderRepository.FindAsync(t => t.OrderNo == eventData.OrderNo);
            entity.Offer((int)EnumCncOrderStatus.OfferComplete, eventData.SellingMoney);
            await _orderRepository.UpdateAsync(entity);
        }

        [UnitOfWork]
        protected virtual async Task<int> ChangeOrderDetailStatusAsync(OfferCncEto eventData)
        {
        ...
        [UnitOfWork]
        protected virtual async Task ChangeOrderMainStatusAsync(OfferCncEto eventData, int id)
        {
