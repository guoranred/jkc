...
namespace Jiepi.InTradeConsumer.Service.Cncs
{
    ...
    /// 订单完成
    /// </summary>
    public class CompleteEventHandler : IDistributedEventHandler<CompleteCncEto>, ITransientDependency
    {
        ...
        public async Task HandleEventAsync(CompleteCncEto eventData)
        {
        ...
        [UnitOfWork]
        protected virtual async Task<bool> ValidAsync(CompleteCncEto eventData)
        {           
            if (eventData == null)
            {
            ...
            if (order.Status >= (int)EnumCncOrderStatus.Finish)
            {
        ...
        [UnitOfWork]
        protected virtual async Task ChangeOrderStatusAsync(CompleteCncEto eventData)
        {
            var entity = await _orderRepository.FindAsync(t => t.OrderNo == eventData.OrderNo);
            entity.SetStaus((int)EnumCncOrderStatus.Finish);
            await _orderRepository.UpdateAsync(entity);
        }

        [UnitOfWork]
        protected virtual async Task<int> ChangeOrderDetailStatusAsync(CompleteCncEto eventData)
        {
