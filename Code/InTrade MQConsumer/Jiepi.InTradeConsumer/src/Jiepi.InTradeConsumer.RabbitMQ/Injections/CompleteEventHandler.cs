...
namespace Jiepi.InTradeConsumer.Service.Injections
{
    ...
    /// 订单完成
    /// </summary>
    public class CompleteEventHandler : IDistributedEventHandler<CompleteInjectionEto>, ITransientDependency
    {
        ...
        public async Task HandleEventAsync(CompleteInjectionEto eventData)
        {
        ...
        [UnitOfWork]
        protected virtual async Task<bool> ValidAsync(CompleteInjectionEto eventData)
        {
            if (eventData == null)
            {
            ...
            if (order.Status >= (int)EnumInjectionOrderStatus.Finish)
            {
        ...
        [UnitOfWork]
        protected virtual async Task ChangeOrderStatusAsync(CompleteInjectionEto eventData)
        {
            var entity = await _orderRepository.FindAsync(t => t.OrderNo == eventData.OrderNo);
            entity.SetStaus((int)EnumInjectionOrderStatus.Finish);
            await _orderRepository.UpdateAsync(entity);
        }

        [UnitOfWork]
        protected virtual async Task<int> ChangeOrderDetailStatusAsync(CompleteInjectionEto eventData)
        {
